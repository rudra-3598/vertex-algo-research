...
def generate_pro_pdf(ticker, mode, data):
    ...
    logo_html = f'<img src="{logo_path}" style="height: 85px;">' if os.path.exists('Black_logo.png') else '<h2>VERTEX ALGO</h2>'
    ...
    if st.button("Generate Premium Report"):
        if selected_ticker and user_email:
            try:
                ...
                st.error("Please configure Email Secrets.")
                return
            ...
            with st.spinner(f"Compiling Institutional Multi-Timeframe Data for {selected_ticker}..."):
                ...
                else: st.error("Data fetch failed. Check API limits or Market Hours.")
        else:
            st.warning("Please select a stock and enter an email.")
