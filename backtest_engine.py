...
def load_fno_base_stocks():
    base_indices = ["NIFTY", "BANKNIFTY"] # Fixed: Always keep indices at the top
...
def fetch_historical_data(fyers, symbol, resolution, days_back):
    ...
    # cont_flag="1" automatically stitches continuous futures data!
...
def run_dixon_backtest(df, use_time_filter=True, skip_tuesday=True):
    ...
    in_trade, trade_type, entry_price, atr_at_entry = False, None, 0, 0
        ...
        if in_trade:
            ...
            if trade_type == "LONG":
                ...
                if trail_active:
                    ...
                    if low <= trail_sl:
                        pnl, cumulative_pnl = trail_sl - entry_price, cumulative_pnl + (trail_sl - entry_price)
                        trade_log.append({"Date": index, "Type": "LONG", "Entry": entry_price, "Exit": trail_sl, "PnL": pnl, "Reason": "Trailing SL"}); in_trade = False
                
                if in_trade and (close < vwap or st_dir == -1):
                    pnl, cumulative_pnl = close - entry_price, cumulative_pnl + (close - entry_price)
                    trade_log.append({"Date": index, "Type": "LONG", "Entry": entry_price, "Exit": close, "PnL": pnl, "Reason": "Hard SL"}); in_trade = False
                ...
                if trail_active:
                    ...
                    if high >= trail_sl:
                        pnl, cumulative_pnl = entry_price - trail_sl, cumulative_pnl + (entry_price - trail_sl)
                        trade_log.append({"Date": index, "Type": "SHORT", "Entry": entry_price, "Exit": trail_sl, "PnL": pnl, "Reason": "Trailing SL"}); in_trade = False
                
                if in_trade and (close > vwap or st_dir == 1):
                    pnl, cumulative_pnl = entry_price - close, cumulative_pnl + (entry_price - close)
                    trade_log.append({"Date": index, "Type": "SHORT", "Entry": entry_price, "Exit": close, "PnL": pnl, "Reason": "Hard SL"}); in_trade = False

        if not in_trade: 
            if row['long_cond']: in_trade, trade_type, entry_price, atr_at_entry, highest_seen, trail_active = True, "LONG", row['close'], row['ATRr_14'], row['close'], False
            elif row['short_cond']: in_trade, trade_type, entry_price, atr_at_entry, lowest_seen, trail_active = True, "SHORT", row['close'], row['ATRr_14'], row['close'], False
...
def run_triple_confluence(df):
    ...
    trade_log, equity_curve, cumulative_pnl = [], [0], 0
    in_trade, trade_type, entry_price, sl, tgt = False, None, 0, 0, 0

    for index, row in df.iterrows():
        if in_trade:
            if trade_type == "LONG":
                if row['low'] <= sl:
                    pnl, cumulative_pnl = sl - entry_price, cumulative_pnl + (sl - entry_price)
                    trade_log.append({"Date": index, "Type": "LONG", "Entry": entry_price, "Exit": sl, "PnL": pnl, "Reason": "Stoploss"}); in_trade = False
                elif row['high'] >= tgt:
                    pnl, cumulative_pnl = tgt - entry_price, cumulative_pnl + (tgt - entry_price)
                    trade_log.append({"Date": index, "Type": "LONG", "Entry": entry_price, "Exit": tgt, "PnL": pnl, "Reason": "Target"}); in_trade = False
            elif trade_type == "SHORT":
                if row['high'] >= sl:
                    pnl, cumulative_pnl = entry_price - sl, cumulative_pnl + (entry_price - sl)
                    trade_log.append({"Date": index, "Type": "SHORT", "Entry": entry_price, "Exit": sl, "PnL": pnl, "Reason": "Stoploss"}); in_trade = False
                elif row['low'] <= tgt:
                    pnl, cumulative_pnl = entry_price - tgt, cumulative_pnl + (entry_price - tgt)
                    trade_log.append({"Date": index, "Type": "SHORT", "Entry": entry_price, "Exit": tgt, "PnL": pnl, "Reason": "Target"}); in_trade = False
            equity_curve.append(cumulative_pnl)
            continue

        if not in_trade:
            if row['long_cond']:
                in_trade, trade_type, entry_price = True, "LONG", row['close']
                sl, tgt = max(row['VWAP_D'], row['EMA_21']) - (row['ATRr_14'] * 0.5), row['close'] + (row['ATRr_14'] * 3.0)
            elif row['short_cond']:
                in_trade, trade_type, entry_price = True, "SHORT", row['close']
...
def render_ui(fyers):
    ...
    st.write("Backtest your proprietary algorithms on NSE Equities/Indices or MCX Continuous Futures.")
    ...
    with col_m2:
        if "NSE" in market_type:
            ...
            auto_expiry = get_current_monthly_expiry() # Fallback MCX expiry logic
            fetch_sym = f"MCX:{test_asset}{auto_expiry}FUT"
            st.caption(f"Fetching Continuous Future: `{fetch_sym}`")
            ...
            if df is not None and not df.empty:
                with st.spinner("Compiling Math & Executing Virtual Trades..."):
                    ...
                    if "Dixon" in selected_strategy:
                        ...
                        logic_text = "ATR Trailing (1x)"
                    else:
                        trades_df, equity_curve = run_triple_confluence(df)
                        logic_text = "Fixed Risk:Reward (1:3)"
                ...
                if not trades_df.empty:
                    ...
                    with m1:
                        st.markdown(f"""<div style="background-color:#1a1c23; padding:15px; border-radius:6px; border: 1px solid #404654; text-align:center;"><h4 style="margin:0; color:#888;">Total Trades</h4><h2 style="margin:0; color:#fff;">{total_trades}</h2></div>""", unsafe_allow_html=True)
                    with m2:
                    ...
                    with m3:
                        ...
                        st.markdown(f"""<div style="background-color:#1a1c23; padding:15px; border-radius:6px; border: 1px solid {pnl_color}; text-align:center;"><h4 style="margin:0; color:#888;">Net PnL (Points)</h4><h2 style="margin:0; color:{pnl_color};">{total_points:+.2f}</h2></div>""", unsafe_allow_html=True)
                    with m4:
                        st.markdown(f"""<div style="background-color:#1a1c23; padding:15px; border-radius:6px; border: 1px solid #1f77b4; text-align:center;"><h4 style="margin:0; color:#888;">Exit Engine</h4><h2 style="margin:0; color:#1f77b4; font-size:16px; margin-top:10px;">{logic_text}</h2></div>""", unsafe_allow_html=True)
                ...
                st.error("Data Fetch Failed. Note: Ensure the API supports this symbol format and timeframe.")
