...
def load_fno_base_stocks():
    if os.path.exists("nse_fno_stocks.csv"):
        try: return ["NIFTY", "BANKNIFTY"] + [t for t in pd.read_csv("nse_fno_stocks.csv")['SYMBOL'].dropna().unique() if t not in ["NIFTY", "BANKNIFTY"]]
        except: return ["NIFTY", "BANKNIFTY", "RELIANCE"]
    return ["NIFTY", "BANKNIFTY", "RELIANCE"]

fno_base_universe = load_fno_base_stocks()
mcx_universe = ["CRUDEOIL", "NATURALGAS", "GOLD", "SILVER"]
...
def calculate_option_pnl(spot_pnl, holding_time_mins, spot_price):
    return (spot_pnl * 0.50) - (holding_time_mins * (spot_price * 0.0000015))

# =====================================================================
# THE NEW V13 GOD MODE BACKTEST ENGINE 🚀🛡️📉
# =====================================================================
        ...
        if trade_type:
            ...
            for opt_idx, opt_candle in window_df.iterrows():
                ...
                if long_conf or short_conf:
                    ...
                    equity_curve.append(cumulative_pnl)
                    break # Sirf 1 entry lenge ek 5-min breakout par
...
def render_ui(fyers):
    ...
    # 👑 Added V13 GOD MODE to dropdown
    ...
    with col_m3:
        test_duration = st.slider("Lookback (Days)", min_value=5, max_value=60, value=30, step=5)
    ...
    if run_btn:
        st.markdown("---")
        with st.spinner(f"Fetching dual-timeframe data and computing math..."):
            if "V13 GOD MODE" in selected_strategy:
                ...
                df = fetch_historical_data(fyers, fetch_sym, "5", test_duration)
                trades_df, equity_curve = run_adaptive_smc(df, is_opt_mode) if df is not None else (pd.DataFrame(), [0])
            else:
                df = fetch_historical_data(fyers, fetch_sym, "15", test_duration)
                trades_df, equity_curve = run_natural_gas_blast(df, is_opt_mode) if df is not None else (pd.DataFrame(), [0])
            ...
            if not trades_df.empty:
                ...
                with m2: 
                    color = "#4caf50" if win_rate >= 32 else "#ff5252" 
                ...
                with m3:
                    ...
                    st.markdown(f"""<div style="background-color:#1a1c23; padding:15px; border-radius:6px; border: 1px solid {pnl_color}; text-align:center;"><h4 style="margin:0; color:#888;">Net PnL</h4><h2 style="margin:0; color:{pnl_color};">{total_points:+.2f}</h2></div>""", unsafe_allow_html=True)
                with m4: 
                    st.markdown(f"""<div style="background-color:#1a1c23; padding:15px; border-radius:6px; border: 1px solid #1f77b4; text-align:center;"><h4 style="margin:0; color:#888;">Stoploss</h4><h2 style="margin:0; color:#1f77b4; font-size:16px; margin-top:10px;">40 Pts (Hard SL)</h2></div>""", unsafe_allow_html=True)
