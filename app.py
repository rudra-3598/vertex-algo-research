        ...
        if st.button("Unlock Terminal"):
            try:
                ...
                if "access_token" in res:
                    ...
                    save_token_to_file(res["access_token"]) # Saves permanently for the day
                    st.success("✅ Token Saved. Refreshing...")
                    st.rerun()
            except Exception as e:
                st.error("Auth Error. Check code.")
...
tab1, tab2, tab3 = st.tabs(["Pro Cash Analyzer", "Live FNO Screener", "Options & Derivatives 🚀"])

# --- ROUTING TO MODULES ---
with tab1:
    pro_analyzer.render_ui(fyers)

with tab2:
    fno_screener.render_ui(fyers)

with tab3:
    options_engine.render_ui(fyers)
